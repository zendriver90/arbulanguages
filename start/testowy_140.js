...
function highlightFirstWord(indexDiv, currentPos, mojeidGlobal) {
    ...
    // +1, bo jeśli currentPos = 0 → pierwsze zdanie, currentPos = 1 → drugie zdanie itd.
...
$("<style>")
  ...
  .appendTo("head");
                            updateHighlight(
            indexDiv, $sentence10, mojeidGlobal, // startowe podświetlenie = pierwsze słowo
...
let sentenceText3 = globalSentence20c + " " + globalSentence22c;
            console.log('funkcja odpala się5', globalSentence20);
            let tempDiv = document.createElement('div');
            let tempDiv2 = document.createElement('div');
                        let tempDiv3 = document.createElement('div');
            tempDiv.innerHTML = sentenceText;
...
function highlightFirstWord3(indexDiv) {
    ...
    console.log('Zawartość kontenera55:', matchingFiszki3);
...
let sentenceText3 = globalSentence30c + " " + globalSentence33c;
            console.log('funkcja odpala się5', globalSentence30);
            let tempDiv = document.createElement('div');
            let tempDiv2 = document.createElement('div');
            let tempDiv3 = document.createElement('div');
            tempDiv.innerHTML = sentenceText;
...
$(document).on("click", ".next-buttonv", function() {

    const indexDiv = $(this).closest('.sentence-line')
                            .find('.sentence-inner')
                            .data('indexdiv');

    if (wordIndexCache[indexDiv] === undefined) wordIndexCache[indexDiv] = 0;
...
$(document).on("click", ".next-buttonv", function() {
    globalWordIndex++;
    syncHighlightAcrossLines();
});

$(document).on("click", ".prev-button", function() {
    if (globalWordIndex > 0) globalWordIndex--;
    syncHighlightAcrossLines();
});
...
const processedLines = lines.map((line, lineIndex) => {
    ...
    const rightBtn = `<button class="next-buttonv next-button${lineIndex + 1}" data-pos="${currentPos}">&gt;</button>`;
...
// --- Obsługa strzałek ---
$(document).on('click', '.prev-button, .next-buttonv', function() {
...
console.log('hej130', id);
            ...
            if (window.systemStarted) {
                sendHighlightToFiszki(window.mojeidGlobal2, currentPosCache[indexDiv], window.mojeidGlobal);
            }
        };
...
if (!sentenceCache[indexDiv][currentPos]) {
    ...
    try {
        ...
        const matchingFiszki3 = fiszki.filter(fiszka => fiszka.id[1] === currentTriplet[2]);
if (currentPos === 0 && typeof addBackgroundToText1b === 'function') {
function attachArrowNavigation($sentenceBlock, indexDiv, currentPos) {
    console.log('%c[attachArrowNavigation] START', 'color: #00aaff', { 
        indexDiv, 
        hasBlock: !!$sentenceBlock.length,
        currentPos: currentPosCache[indexDiv]
    });

    if (!$sentenceBlock || $sentenceBlock.length === 0) {
        console.warn('[attachArrowNavigation] Brak $sentenceBlock dla indexDiv:', indexDiv);
        return;
    }
    ...
    const $container = $(`.image-container3b[data-lesson="${indexDiv}"]`);
                    ...
                    displayWords(currentWordIndex, indexDiv); // WywoĹanie z indekse /// przekaż do funkcji displayWords moje mojeidWordIndex