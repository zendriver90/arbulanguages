...
function generateFiszkaBlock(fiszka, lessonId2, category) {
    ...
    const matchingFiszki = fiszki.filter(f => f.id[1] === lessonId2);

    if (!matchingFiszki.length) return;

    const multiCount = countMultiVersionFiszkiForLesson(fiszki, lessonId2);

    function loadWholeLesson() {

        $('.grid-containerb').empty(); // 🔥 czyścimy przed ładowaniem

        matchingFiszki.forEach(fiszka => {
            generateFiszkaBlock(fiszka, lessonId2, category);
        });
    }

    // 🔴 JEŚLI SĄ MULTI → CZEKAJ NA WYBÓR
    if (multiCount > 0 && !isLessonLoading) {
        ...
        $('#modeRandom').off('click').one('click', function () {
            MATRIX_MODE = 'RANDOM';
            $('#multiVersionOverlay').fadeOut();
            isLessonLoading = true;
            loadWholeLesson(); // 🔥 dopiero teraz ładuje się strona
        });

        $('#modeApprove').off('click').one('click', function () {
            MATRIX_MODE = 'APPROVE';
            $('#multiVersionOverlay').fadeOut();
            isLessonLoading = true;
            loadWholeLesson(); // 🔥 dopiero teraz ładuje się strona
...
// Sprawdzamy, czy fiszka ma wiele wersji
...
if (isMultiVersion) {
    const $btnApprove = $('<button>')
        ...
        .css({ marginRight: '4px' })
        .on('click', () => {
            console.log('Kliknięto Zatwierdź!');
            // Tutaj dodaj push do historii + odświeżenie matrycy
        });
