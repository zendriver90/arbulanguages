...
`);
    ...
    if (index55b2 === 3) { // Porównaj jako liczby
        ...
        window.location.href = 'demo3esp.html?category=' + encodeURIComponent(globalCategory);
            ...
            let tablica33 = [];
            let tablica33b = [];
            function wybierzRodzaj(category, czas, matchingIndexes, index55, buttonindex, idpolski, frazyid, matchingIndexes2, matchingIndexes4, rodzaj, matchingIndexes3) {
        ...
        <script>
            function blinkButton() {
                ...
                const interval = setInterval(() => {
                    ...
                    if (blinkCount === 6) { // 6 przełączeń widoczności (3 mignięcia)
                        clearInterval(interval);
                        button.style.visibility = 'visible'; // Upewnij się, że przycisk jest widoczny na końcu
                    }
                }, 300); // 300 ms przerwy pomiędzy mignięciami
            }
...
`);
            ...
            function przekazArgument0(category, index55, buttonindex, czas, idpolski, idtranslate, id) {
                ...
                wybierzCharakter(category, tablica3[0], index55, buttonindex, tablica3[1], tablica3[3]);
            }
            ...
            function wybierzCharakter(category, czas, index55, buttonindex, idpolski, frazyid) {
                ...
                wybierzRodzaj(globalCategory, czas, matchingIndexes, index55, buttonindex, idpolski, frazyid);
            ...
            function findXForMatchingFiszka(category, fiszki) {
                ...
                for (let i = 0; i < fiszki.length; i++) {
                    ...
                    } else if (fiszki[i] && fiszki[i].category3 && fiszki[i].category3.includes(category)) {
                        matchingIndexes.push(i + 1);
                        console.log('hej7');
                    }
                }
...
// Sprawdź czy tablica indexes zawiera zero
                        ...
                        if (shouldInsertNumbers) {
                            ...
                            let secondValue = indexes.length > 0 ? indexes[1] : firstValue + 1;
                           ...
                           sentence30 = "";
                                ...
                                for (let ki = 0; ki < fiszka.sentence1.length; ki++) {
                                    ...
                                    if (aktualny) {
                                        if (matchingIndexes.includes(indexToCheck) && currentIndexValue == 0) {
                                            const spanElement = document.createElement('span');
                                            spanElement.textContent = sentencePart;
                                            spanElement.classList.remove('highlighted-sentence1');
                                            spanElement.classList.add('yellow');
                                            // Zaktualizuj sentencePart, aby zawierał utworzony element span
                                            sentencePart = spanElement.outerHTML;
                            ...
                            if (sentence1b2 === "") {
                                ...
                                sentence1b2 += sentencePart333 + " ";
                                            ...
                                            const spanElement = document.createElement('span');
                                            spanElement.textContent = sentencePart;
                                            spanElement.classList.add('highlighted-sentence1');
                                            // Zaktualizuj sentencePart, aby zawierał utworzony element span
                                            sentencePart = spanElement.outerHTML;
...
if (!stopButtonAdded) {
    ...
    } else if (indexDiv === 1 || indexDiv % 3 === 0 || indexDiv % 5 === 0 || indexDiv % 7 === 0) {
...
`);
                    ...
                    $button.on('click', function () {
                        ...
                        setTimeout(() => {
                            ...
                            przekazArgument0(tablica3[2], index55, true, tablica3[0], true, true, tablica3[1]);
                        }, 100);
...
`);
                    ...
                    $buttonb2.css({
                        'position': 'absolute',
                        'left': '70px', // Przykładowa wartość odstępu od lewej krawędzi
                        'top': '170px',
                        'height': '20px',
                        'z-index': '99999999' // Ustawienie z-index dla buttona większe niż dla sentenceDiv
                    });
                    $container.append($buttonb2);
                    // Dodanie diva z zdaniem do kontenera
                    $container.append($sentenceDiv);
...
`);
            ...
            function countVisibleContainers() {
                ...
                if (indexes.length >= 8 && elapsedTime >= 400) {
                    ...
                    if (elapsedTime >= 300) {
                        ...
                        otherFunction(indexes);
                    } else if (elapsedTime >= 1000 && funkcjaWywolana) {
                        // Ustaw timeout na 300ms po spełnieniu warunku
                        timeoutId = setTimeout(() => {
                            previousVisibleIndexes = [...indexes]; // kopiowanie tablicy indexes do previousVisibleIndexes
                            console.log('Aktualne widoczne indeksy2:', indexes);
                    ...
                    } else if (indexes.length < 8 && timeoutId) {
            ...
            $(window).on('scroll', function () {
                ...
                debounceTimer = setTimeout(function () {
                    ...
                    countVisibleContainers();
                }, 200); // Czas w milisekundach, po którym funkcja zostanie wywołana
            });
...
// Funkcja otherFunction z przekazanymi indeksami
            function otherFunction(indexes) {
                ...
                przekazArgument0(tablica3[2], undefined, false, 'zdania', false, false, tablica3[1]);
            }
            przekazArgument0(tablica3[2], '', false, 'fiszki', true, true, tablica3[1]);
            ustawTryb('fiszki');
        </script>
        <script id="demoScript" data-category=""></script>
            ...
            function wybierzRodzaj(category, czas, matchingIndexes, index55, buttonindex, idpolski, frazyid, matchingIndexes2, rodzaj, matchingIndexes3) {
                ...
                if (isSearching) {
                    ...
                    myVariable = moja tablica którą chce przekazać;
                }