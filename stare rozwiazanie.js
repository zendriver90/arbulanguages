...
// Utwórz element <source> dla wideo i ustaw atrybuty
                            const $sourceElement = $('<source>').attr({
                                'src': tablica10[0], // Poprawne użycie interpolacji
...
// Dodaj element video do diva dla wideo
    ...
    const srcWords = [srcWord1, srcWord2, srcWord3];
          ...
          // Deklaracja tablicy, która będzie zawierać wszystkie tablice
                ...
                // Sprawdź, czy tablica10 nie jest pusta
                console.log('hej22', tablica10);
                 setTimeout(() => {
if (tablica10.length > 0) {
    // Losowo wybierz indeks tablicy z tablica10
    const randomArrayIndex = Math.floor(Math.random() * tablica10.length);
    const selectedArray = tablica10[randomArrayIndex];

    // Upewnij się, że wybrana tablica nie jest pusta ani undefined
    if (selectedArray && selectedArray.length > 0) {
        // Użyj pierwszego elementu wybranej tablicy jako index
        const index = selectedArray[0];

        // Wyświetl wybrany indeks w konsoli
        console.log('Wybrany indeks:', index);

        // Wywołaj funkcję addVideo z uzyskanym indeksem
        addVideo(0);
    } else {
        console.error('Wybrana tablica jest pusta lub undefined.');
    }
} else {
    console.error('Tablica tablica10 jest pusta.');
}
        }, 900);
