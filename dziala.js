...
console.log('111', matchingFiszki1);
console.log('222', matchingFiszki2);
console.log('333', matchingFiszki3);
...
function pobierzSrcWordDlaLekcji(lessonId, fiszki) {
    ...
    return matchingFiszka ? matchingFiszka.srcWord : null;
                        ...
                        for (let i = 0; i < matchingFiszki1.length; i++) {
                            ...
                            if (matchingIndexes.includes(i + matchingFiszki3.length + 1 + startIndex)) {
                                sentencePart = `<span style="color: yellow;">${fiszka.sentence[i % 3]}</span>`;
                                console.log(`Zaznaczono słowo ${i + matchingFiszki3.length + 1 + startIndex} w pierwszym zestawie fiszek.`);
                            }
                        ...
                        for (let j = 0; j < matchingFiszki2.length; j++) {
                            ...
                            // Sprawdzenie, czy indeks bieżącej fiszki znajduje się w tablicy matchingIndexes
                            if (matchingIndexes.includes(lessonIdToShow2*3 + j + 1 + startIndex)) {
                                sentencePart = `<span style="color: yellow;">${fiszka.sentence[j % 3]}</span>`;
                                console.log(`Zaznaczono słowo ${lessonIdToShow2*3 + j + 1 + startIndex} w drugim zestawie fiszek.`);
                            }
                        ...
                        for (let k = 0; k < matchingFiszki3.length; k++) {
                            ...
                            // Sprawdzenie, czy indeks bieżącej fiszki znajduje się w tablicy matchingIndexes
                            if (matchingIndexes.includes(matchingFiszki2.length*3 + k + 1 + startIndex)) {
                                sentencePart3 = `<span style="color: yellow;">${fiszka.sentence[k % 3]}</span>`;
                                console.log(`Zaznaczono słowo ${matchingFiszki2.length*3 + k + 1 + startIndex} w trzecim zestawie fiszek.`);
                                console.log('hej15', matchingFiszki2.length);
                            }
