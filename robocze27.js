...
function calculatePartLengths(fiszki) {
    ...
    fiszki.forEach(fiszka => {
        ...
        if (!fiszka.sentence1) return; // Zabezpieczenie przed obiektami fiszki, które nie mają właściwości sentence2
        ...
        if (!idToLengthsMap.has(y)) {
            ...
            // Sprawdzamy, czy sentence2 jest ciągiem znaków lub tablicą słów
            const sentence1Array = Array.isArray(fiszka.sentence1) ? fiszka.sentence1 : fiszka.sentence1.split(' ');

            sentence1Array.forEach(word => {
            ...
            const secondPartLength = sentence1Array.length - repeatedWords;
...
function calculatePartLengths2(fiszki) {
    ...
    fiszki.forEach(fiszka => {
        ...
        if (!fiszka.sentence2) return; // Zabezpieczenie przed obiektami fiszki, które nie mają właściwości sentence2
        ...
        if (!idToLengthsMap.has(y)) {
            ...
            // Sprawdzamy, czy sentence2 jest ciągiem znaków lub tablicą słów
            const sentence2Array = Array.isArray(fiszka.sentence2) ? fiszka.sentence2 : fiszka.sentence2.split(' ');

            sentence2Array.forEach(word => {
                if (uniqueWords.has(word)) {
                    repeatedWords++;
                } else {
                    uniqueWords.add(word);
            ...
            const firstPartLength = sentence2Array.length - repeatedWords;
                ...
                const lessonObject = {
                    ...
                    lesson3SecondPartLength: lesson3SecondPartLength
                                ...
                                if (tablica10.length <= 3 || tablica10.length === 0) {
                                    ...
                                    showCombinedSentenceForLesson(lessonId1, lessonId2, lessonId3, fiszki, matchingIndexes, startIndex, newIndex, indexDiv, lessonsArray, lesson1Sentences, lesson2Sentences, lesson3Sentences, lesson1FirstPartLength, lesson2FirstPartLength, lesson3FirstPartLength, lesson1SecondPartLength, lesson2SecondPartLength, lesson3SecondPartLength);
...
// Pobierz parametr z adresu URL
                ...
                function showCombinedSentenceForLesson(lessonIdToShow1, lessonIdToShow2, lessonIdToShow3, fiszki, matchingIndexes, startIndex, newIndex, indexDiv, lessonsArray, lesson1Sentences, lesson2Sentences, lesson3Sentences, lesson1FirstPartLength, lesson2FirstPartLength, lesson3FirstPartLength, lesson1SecondPartLength, lesson2SecondPartLength, lesson3SecondPartLength) {
...
console.log('hej450', lessonIdToShow2);
                    if (matchingFiszki1.length > 0 && matchingFiszki2.length > 0 && matchingFiszki3.length > 0) {
                        ...
                        function addBackgroundToText(matchingFiszki1, lessonIdToShow1, matchingIndexes, currentIndexValue, aktualny, lesson1Sentences, lesson1FirstPartLength, lesson1SecondPartLength) {
                            $(document).ready(function () {
                                setTimeout(() => {
                                sentence10 = "";

                            for (let ii = 0; ii < matchingFiszki1.length; ii++) {
                                console.log('hej335', matchingFiszki1);
                                const fiszka = matchingFiszki1[ii];
                                console.log('hej333', fiszka);
                                console.log('hej300', lesson1FirstPartLength);
                                const sentence1 = fiszka.sentence1.join(" "); // Łączy trzy słowa w jedno zdanie
                                console.log('hej43', sentence1);
    // Deklaracja zmiennej sentence2 na zewnątrz pętli forEach
    let sentence2 = '';

    // Używanie forEach do przetwarzania sentence2
    matchingFiszki1.forEach(f => {
        if (f.sentence2) {
    ...
    console.log('hej27', combinedSentence);
                                ...
                                if (aktualny) {
                                    if (matchingIndexes.includes(ii + (lessonIdToShow1 - 1) * lesson1Sentences[1] + 1) && currentIndexValue == 0) {
                                        ...
                                        console.log(`Zaznaczono słowo ${ii + (lessonIdToShow1 - 1) * 6 + 1} w pierwszym zestawie fiszek.`);
                                        ...
                                        // Zaktualizuj sentencePart, aby zawierał utworzony element span
                                        combinedSentence = spanElement.outerHTML;
                                        console.log(`Zaznaczono słowo ${ii + (lessonIdToShow1 - 1) * lesson1Sentences[1] + 1} w pierwszym zestawie fiszek.`);
                                    } else if (currentIndexValue == 1 && matchingIndexes.includes(ii + (lessonIdToShow1 - 1) * lesson1Sentences[1] + 1)) {
                                        const spanElement = document.createElement('span');
                                        spanElement.textContent = combinedSentence;
                                ...
                                sentence10 += combinedSentence + " ";

                            }
                            $sentenceDiv.html('<span class="sentence10">' + sentence10 + '</span><br>' + '<span class="sentence20">' + sentence20 + '</span><br>' + '<span class="sentence3" style="position:relative;">' + sentence3 + '</span><br>');
                        ...
                        let sentence1bb = "";
                        let sentence1bba = "";
                        function addBackgroundToText1b(matchingFiszki1, lessonIdToShow1, matchingIndexes, aktualny, lesson1Sentences) {
                            $(document).ready(function () {
                        ...
                        function addBackgroundToText2(matchingFiszki2, lessonIdToShow2, matchingIndexes, currentIndexValue, aktualny, lesson2Sentences, lesson2FirstPartLength, lesson2SecondPartLength) {
                                                                $(document).ready(function () {
                            sentence20 = "";
                                                            setTimeout(() => {
                            for (let j = 0; j < matchingFiszki2.length; j++) {
                                const fiszka = matchingFiszki2[j];
                                console.log('hej340', fiszka);
                                const sentence1 = fiszka.sentence1.join(" "); // Łączy trzy słowa w jedno zdanie
                                console.log('hej43', sentence1);
    // Deklaracja zmiennej sentence2 na zewnątrz pętli forEach
    let sentence2 = '';
    ...
    const combinedSentence = sentence1 + " " + sentence2;
    console.log('hej27', combinedSentence);
    console.log('hej27', lesson2Sentences[1]);
                        ...
                        let sentence1bb2 = "";
                        let sentence1bb2a = "";
                        function addBackgroundToText2b(matchingFiszki2, lessonIdToShow2, matchingIndexes, aktualny, lesson2Sentences) {
                        ...
                        function addBackgroundToText3(matchingFiszki3, lessonIdToShow3, matchingIndexes, currentIndexValue, aktualny, lesson3Sentences, lesson3FirstPartLength, lesson3SecondPartLength) {
                                    $(document).ready(function () { 
                                        sentence3 = "";
                                                            setTimeout(() => {
...
let sentencePart1 = fiszka.sentence1[k % lesson3FirstPartLength]; // Pierwszy człon zdania
let sentencePart2 = fiszka.sentence2[k % lesson3SecondPartLength];
let combinedSentence = sentencePart1 + " " + sentencePart2;
console.log('hej301', combinedSentence);
                                // Sprawdź, czy indeks jest równy 0
                                if (aktualny) {
                                    ...
                                    $videoElement.on('timeupdate', function () {
                                        ...
                                        if (!currentIndexValue.includes(index)) {
                                            ...
                                            if (currentIndexValue[0] === 0 || currentIndexValue[0] === 1) {
                                                addBackgroundToText(matchingFiszki1, lessonIdToShow1, matchingIndexes, currentIndexValue, true, lesson1Sentences, lesson1FirstPartLength, lesson1SecondPartLength);
                                                addBackgroundToText2(matchingFiszki2, lessonIdToShow2, matchingIndexes, currentIndexValue, true, lesson2Sentences, lesson2FirstPartLength, lesson2SecondPartLength);
                                                addBackgroundToText3(matchingFiszki3, lessonIdToShow3, matchingIndexes, currentIndexValue, true, lesson3Sentences, lesson3FirstPartLength, lesson3SecondPartLength);
                                            }
...
// Tworzenie nowego elementu div za pomocą jQuery
                        ...
                        $sentenceDiv.html(`${sentence10}<br>${sentence20}<br>${sentence3}`); // Dodanie treści do diva
                        $sentenceDiv.css({
                            'position': 'absolute',
                            'font-size': '18px',
                            'bottom': '0',
